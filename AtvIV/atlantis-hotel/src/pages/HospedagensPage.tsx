...
import { getHospedagens, getHospedagensAtivas, checkOut } from "../api/hospedagens";
                            ...
                            {hospedagensAtivas.map(hospedagem => (
                                <div key={hospedagem.id} className="bg-white rounded-xl shadow-lg p-6">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className="text-lg font-semibold text-gray-800">{hospedagem.cliente.nome}</h3>
                                            <p className="text-gray-600 mt-1">
                                                <span className="font-medium">Acomodação:</span> {hospedagem.acomodacao.nome}
                                            </p>
                                            <p className="text-gray-600 mt-1">
                                                <span className="font-medium">Check-in:</span> {new Date(hospedagem.dataCheckIn).toLocaleString()}
                                            </p>
                                            <p className="text-gray-600 mt-1">
                                                <span className="font-medium">Valor de Estadia: R$</span>{hospedagem.valorTotal}
                                            </p>
                                            {hospedagem.dependentes.length > 1 && (
                                                <p className="text-gray-600 mt-1">
